...
const i18n = {
  ...
  tr:{
    ...
    forgotPassword:"Şifremi Unuttum?", forgotPasswordTitle:"Şifre Sıfırla",
    forgotPasswordDesc:"E-postanızı girin, size sıfırlama bağlantısı gönderelim.",
    forgotPasswordSent:"Şifre sıfırlama e-postası gönderildi! Gelen kutunuzu kontrol edin.",
...
export default function App() {
  ...
  const handleRegister=async()=>{
    ...
    if(regForm.username.trim().length<3){showToast("Username must be at least 3 characters.","error");return;}

    // Step 1: Create auth user
    const {data,error}=await supabase.auth.signUp({
      email:regForm.email,
      password:regForm.password,
      options:{ data:{ username:regForm.username } }
    });
    if(error){showToast(error.message,"error");return;}

    // Step 2: Insert profile — works whether email confirmation is on or off
    ...
    // If disabled, they're already logged in
    if(data?.session){
      showToast(`${t.welcome}, ${regForm.username}!`);
    } else {
  ...
  const handleForgotPassword=async(email)=>{
    if(!email||!email.includes("@")){showToast(t.emailRequired,"error");return;}
    const {error}=await supabase.auth.resetPasswordForEmail(email,{
      redirectTo:`https://dunyadaturk.com/reset-password`,
    });
    if(error){showToast(error.message,"error");}
    else{showToast(t.forgotPasswordSent);setModal(null);}
  };
  ...
  return (
    <div style={S.page}>
        ...
        <div style={S.modal} onClick={e=>{if(e.target===e.currentTarget)setModal(null);}}>
          <div style={S.modalBox}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
              <label style={{...S.label,marginBottom:0}}>{t.password}</label>
              <span style={{fontSize:12,color:COLORS.primary,cursor:"pointer",fontWeight:600}} onClick={()=>setModal("forgotPassword")}>{t.forgotPassword}</span>
            </div>
      ...
      {modal==="forgotPassword"&&(
        <ForgotPasswordModal t={t} S={S} COLORS={COLORS} onSend={handleForgotPassword} onBack={()=>setModal("login")} onClose={()=>setModal(null)}/>
      )}
...
function ForgotPasswordModal({t,S,COLORS,onSend,onBack,onClose}){
  const [email,setEmail]=useState("");
  const [sending,setSending]=useState(false);
  return(
    <div style={S.modal} onClick={e=>{if(e.target===e.currentTarget)onClose();}}>
      <div style={S.modalBox}>
        <h2 style={{margin:"0 0 8px",fontSize:22,fontWeight:800}}>🔒 {t.forgotPasswordTitle}</h2>
        <p style={{margin:"0 0 20px",fontSize:14,color:COLORS.textMuted}}>{t.forgotPasswordDesc}</p>
        <label style={S.label}>{t.email}</label>
        <input
          ...
          onKeyDown={e=>e.key==="Enter"&&!sending&&(setSending(true),onSend(email).finally(()=>setSending(false)))}
          autoFocus
        />
        <div style={{display:"flex",gap:10}}>
          <button
            style={{...S.btn("primary"),flex:1,opacity:sending?0.7:1}}
            disabled={sending}
            onClick={()=>{setSending(true);onSend(email).finally(()=>setSending(false));}}
          >
            {sending?"⏳ ...":t.sendResetLink}
          </button>
          <button style={S.btn("outline")} onClick={onBack}>← {t.backToLogin}</button>
        </div>
      </div>
    </div>
  );
